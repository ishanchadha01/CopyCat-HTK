...
/**
 * A constraint which determines that child annotations always cover the full
 * time interval of the parent annotation, without allowing gaps. The sequence
 * of depending annotation form a directed graph from parent begin to parent 
 * end.
 * @see TimeSubdivision
 */
public class NoTimeGapWithinParent extends ConstraintImpl {
    ...
    /**
     * @return the constant {@link Constraint#NO_GAP_WITHIN_PARENT}
     */
    @Override
     ...
     * mechanism applied here sometimes conflicts with the graph based
     ...
     * end). To accomplish this time values of TimeSlots are altered.  But
     ...
     * To (temporarily) solve this an existing annotation is now sometimes
     * marked for deletion here, which looks odd. This whole mechanism 
     * maybe should be redesigned.
     *
    ...
    */
    ...
    /**
     * Enforces this constraint by iterating over all annotations of the parent
     * tier, find enclosed dependent annotations per parent and check if the
     * dependent annotations are correctly connected. 
     *
     * @param theTier the dependent tier to check and correct
     */
    @Override
    ...
    /**
     * Creates a new annotation and time slots (if necessary), adds it to the
     * tier and registers the annotation with the parent.
     *  
     * @param beforeAnn insert a new annotation before this one 
     ...
     * tier and registers the annotation with the parent.
     * 
     * @param afterAnn insert an annotation after this one
     * @param theTier the target tier
     *
     * @return the new annotation or null
     */
	...
	public void detachAnnotation(Annotation theAnn, TierImpl theTier) {
        ...
        Iterator<Annotation> childIter = childAnnots.iterator();

        while (childIter.hasNext()) {
            Annotation ann = childIter.next();
     ...
     * a child of the specified parent annotation.
