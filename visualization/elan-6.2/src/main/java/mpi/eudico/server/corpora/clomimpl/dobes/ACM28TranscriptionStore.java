...
public class ACM28TranscriptionStore implements TranscriptionStore {
	...
	public void loadTranscription(Transcription theTranscription, DecoderInfo decoderInfo) {
		...
		*/
        ...
        if (linguisticTypes == null) {
        	linguisticTypes = new ArrayList<LingTypeRecord>(0);
        }
 ...
 //           typesCopy.add(i, linguisticTypes.get(i));
        ...
        List<String> parsedTierNames = parser.getTierNames(trPathName);
        if (!attisTr.isLoaded() && parsedTierNames != null) {
            Iterator<String> iter = parsedTierNames.iterator();
...
//        		}
	 ...
	 * 
	 * @param transcription the transcription to load
	 * @param parser the parser to get the reference links from
	 */
