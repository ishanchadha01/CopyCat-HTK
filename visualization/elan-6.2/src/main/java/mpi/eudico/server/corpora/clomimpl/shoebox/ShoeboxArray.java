...
/**
 * For parsing the shoebox file, I first read it into an array because:  - Each
 * line of the shoebox file has to be addressed over and over again -
 * shoebox file is organized into blocks and tiers, which translate to rows and
 * columns. - Shoebox wraps a a line at 80 characters. As a parse
 * preprocessing, the hard wrap of the shoebox file is removed. For
...
//    public static String label_eudicot2 = "\\EUDICOt2";
    ...
    // store the previous label name to determine the place in the list
...
//        logger.log(Level.FINE, " --- getCell(" + label + ", " + block);
	 ...
	 * 
	 * @return the time in seconds, -1 if the time string has an illegal
	 *         format
	 */
    ...
    /**
     * 
     * @param labelincludingtrailingbackslash  the input string, e.g. "\tx"
     *
     * @return the index in the list of labels or -1 if not found
     */
...
//        logger.log(Level.FINE, "     prepare_block()");
    ...
    /**
     * side effect on variables: currentIndexLabel, currentIndexBlock stores the
     * content for this label in shoeboxArray. if label is \per, store name
     * separately (additionally) in shoeboxArray.
     *
