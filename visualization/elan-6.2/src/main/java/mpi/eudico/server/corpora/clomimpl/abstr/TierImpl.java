 ...
 * @version Dec 2006 getter and setter for "Annotator" added
 */
public class TierImpl implements Tier {
	...
	/**
	 * Delegates handling of the modification to the transcription.
	 */
    @Override
	 ...
	 * @param transcription the parent transcription
	 * @param theType the linguistic type for 'this' tier
	 */
	public TierImpl (TierImpl parenttier, String name, String participant, 
			TranscriptionImpl transcription, LinguisticType theType) {
	 ...
	 * Arguments {@code beginTime} and {@code endTime} can either be different or equal.
	 * By convention the latter is the case for, for example, creation of  
	 * new RefAnnotations.
	 * 
	 * @param beginTime the begin time
	 * @param endTime the end time
	 * @return a new annotation, alignable or reference or {@code null}
	 */
	 ...
	 * 
	 * <p>A single tier may contain either Alignable- or RefAnnotations.
	 ...
	 * </p>
	 * @author MK 18/06/02
	 * @param theAnnotation the annotation to add
	 */
	...
	/**
	 * Adds the annotation to the annotations {@code TreeSet} without further
	 * checks. Called as part of the process of loading from file, shouldn't be
	 * public, ideally.    
	 * 
	 * @param theAnnotation the annotation to insert
	 */
	...
	/**
	 * Calls {@link Constraint#insertBefore(Annotation, TierImpl)} if the 
	 * constraint of this tier's type supports this.
	 * 
	 * @param beforeAnn the annotation to insert one before
	...
	/**
	 * Calls {@link Constraint#insertAfter(Annotation, TierImpl)} if the 
	 * constraint of this tier's type supports this.
	 * 
	 * @param afterAnn the annotation to insert one after
	 * @return a new annotation or {@code null}
	 */
	...
	/**
	 * Removes the annotation by first calling its {@link Annotation#markDeleted(boolean)} 
	 * method and then calling the transcription's {@link TranscriptionImpl#pruneAnnotations(Tier, Annotation)}.
	 */
	@Override
	 ...
	 * are indeed that.
	 * 
	 * @return a list containing {@code AlignableAnnotation}s
	 */
	 ...
	 * are indeed that.
	 * 
	 * @return a list containing {@code RefAnnotation}s
	 */
	...
	/**
	 * A {@code Map<String, Annotation>} that is passed to this method will be
	 * populated with annotation id to annotation key-value pairs.
	 * 
	 * @param map the map to which annotations should be added with their
	 * id as key, not {@code null}
	 */
	...
	/**
	 * @param theAnnotation the query annotation
	 * @return the annotation which comes before the specified annotation in the
	 * {@code TreeSet} of annotations, or {@code null}
	 */
	...
	/**
	 * @param time the query time value
	 * @return the last annotation with an end time less than or equal to the
	 * requested time, or {@code null}
	 */
	...
	/**
	 * @param theAnnotation the query annotation
	 * @return the annotation after the specified annotation in the {@code TreeSet}
	 * of annotations, or {@code null}
	 */
	...
	/**
	 * @param time the query time value 
	 * @return the first annotation with an end (!) time greater than the
	 * requested time
	 */
...
*/
	...
	/**
	 * Corrects time alignment of annotations that overlap the specified 
	 * annotation. This is part of the process of applying constraints after
	 * creation of a new annotation or modification of (the boundaries of) an
	 * existing annotation.
	 * @param fixedAnnotation the new or modified annotation, which should not
	 * be modified itself in this process
	 */
	public void correctTimeOverlaps(AlignableAnnotation fixedAnnotation) {
	...
	/**
	 * @param connectedAnnotList a list of annotations to find the endpoints for
	 * @return the begin and end time slot of the root annotation or top most
	 * annotation in the specified list
	 */
	...
	/**
	 * Returns the time slots where the 'subchain' containing theSlot connects to
	 * either an aligned time slot or an unaligned slot shared with a parent annotation.
	 * 
	 * @param theSlot the query time slot
	 * @param stopAtAlignedSlot if true the search stops at an aligned slot
	 * @return the begin and end time slot of the subchain
	 */
	 ...
	 * 
	 * @param theSlot the query slot
	 * @param stopAtAlignedSlot if true stop the search at an aligned slot
...
*/	
	 ...
	 * 
	 * @param fixedAnnotation the source annotation, should not be moved
	 * @param oldBegin the old begin time of the annotation, used to calculate
	 * the left side shift 
	 * @param oldEnd the old end time, used to calculate the right side shift
	 */
...
//						forceChildAnnotationsOfAlignable(depAA);
	...
	/**
	 * <p>Dynamic retrieval of tier properties, which can be of any type.
	 ...
	 * <p>
	 * Note: this method does not check if there are already annotations and, if so,
	 * if they are compatible with the new type. It could throw an exception in case of
	 * incompatibilities.
	 * @param theType the new type for this tier, not {@code null}
	 */
	@Override
	public void setLinguisticType(LinguisticType theType) {
		linguisticType = theType;
		updateCVEntryIds();
		// HS why not pass the type as the modification here?
		modified(ACMEditEvent.CHANGE_TIER, null);
	}
	...
	/**
	 * Could return {@code null} if not set, instead of the default value
	 * @author MK 21/06/02
	 * @return the participant of this tier or a default value ("not specified")
	 */
	@Override
	 ...
	 * @return the {@code annotator} or an empty string
	 */
	@Override
	 ...
	 * 
	 * @param annotator the annotator id or name
	 */
	@Override
	...
	/**
	 * The Locale is used for possibly available input methods.
	 *  
	 * @author MK 21/06/02
	 * @return	the locale of this tier or {@code null} if not set
	 * @see #getLangRef()
	 */
	...
	/**
	 * 
	 * @param l the Locale for this tier, can be set to {@code null}
	 * @see #getDefaultLocale()
	 * @see #setLangRef(String)
	 */
	 ...
	 * it's unsafe.
	 * HS Aug. 2011: removed obsolete constraint checks and updating of 
	 * annotations (they didn't work). Now only the parent is set and an
	 * ACMEditEvent is generated. 
	 * <p>
	 * Note: should throw an exception if there are already annotation on
	 * this tier and/or if the types of the tiers are not compatible
	 * @param newParent may or may not be {@code null}
	 */
	...
	/**
	 * @return {@code true} if this tier has a parent tier in the current 
	 * transcription, {@code false} otherwise
	 */
	@Override
	...
	/**
	 * @return the root tier of the tier hierarchy this tier is part of
	 * or {@code this} if this tier has no parent tier
	 * */
	@Override
	 ...
	 * The comparison of times is begin time inclusive, end time exclusive!
	 * 
	 ...
	 * @return the annotation at the specified time, or {@code null}
	 */
	...
	/**
	 * 
	 * @param theTime the time to look for
	 * @return the annotation at that time or {@code null}
	 * @see #getAnnotationAtTime(long, boolean)
	 */
...
*/
	...
	/**
	 * @param t1 the begin time of the interval
	 * @param t2 the end time of the interval
	 * @return a list of annotations (of this tier), not {@code null} but can be empty
	 */
	...
	*/
	...
	/**
	 * A method to get the overlapping annotations (of this tier) based on the
	 * timeslots order.<br>
	...
	/**
	 * A method to get the overlapping annotations (of this tier) based on the timeslots order.<br>
	...
	/**
	 * @param theSlot the time slot
	 * @return a list of all annotations (of this tier) that hold a reference to the specified
	 *  time slot, for either begin or end boundary
	 */
	 ...
	 * @param theSlot the time slot
	 * @return a list of all annotations (of this tier) that hold a reference to the specified
	 *  time slot for their begin time
	 */
	...
	/**
	 * @return {@code true} if the linguistic type (tier type) is time alignable
	 * @see LinguisticType#isTimeAlignable()
	 */
	 ...
	 * ON THIS TIER.
	 *
	 * @param theSlot the slot to calculate the time for
	 * @return proposed time for the slot
	 */
	...
	//////////////////////
	/**
	 * Propagate changes to depending tiers after creation of a new annotation
	 * on a time subdivision tier.
	 ...
	 * @see #getAnnotsChainEndingWithSlot(ArrayList, TierImpl, TimeSlot, TimeSlot)
	 * @param annots the list where found annotations are stored
	 * @param tier the tier containing the annotations to query
	 * @param bts begin time slot
	 * @param ets the end slot of the chain
	 */
	private void getAnnotsChainStartingWithSlot(ArrayList<AlignableAnnotation> annots, TierImpl tier, 
			TimeSlot bts, TimeSlot ets) {
	 ...
	 * @see #getAnnotsChainStartingWithSlot(ArrayList, TierImpl, TimeSlot, TimeSlot)
	 * @param annots the list where found annotations are stored
	 * @param tier the tier containing the annotations to query
	...
	public interface ValueGetter {
		 ...
		 * 
		 * @param t the tier to get the specific value from
		 * @return the value or an empty string (rather than {@code null})
		 */
