...
/**
 * A panel for showing and editing the media files that are linked to or 
 * associated with the transcription.
 * A table shows a list of linked files, their locations and their status (found
 * or not found). If a file has been renamed or has been moved it can be updated
 * here. Linked files can be removed and new files can be added. 
 * 
 * @author Han Sloetjes
 */
...
public class MediaFilesPanel extends JPanel implements ActionListener,
	...
	/**
	 * Creates a new MediaFilesPanel.
	 *
	 * @param transcription the transcription
	 * @param linkedFilesDialog the parent or owner dialog
	 */
...
outerloop: 
	...
	private void addRemoteMedia() {
        ...
        if (url == null) {
            ...
            MediaDescriptor md = MediaDescriptorUtility.createMediaDescriptor(url);
	...
	private void addMediaDescriptor() {
		...
		MediaDescriptor md = MediaDescriptorUtility.createMediaDescriptor(file);
	...
	private void addMediaDescriptor(MediaDescriptor md) {
		...
		for (int i = 0; i < currentMDCopy.size(); i++) {
			...
			if (otherMD.mimeType.equals(MediaDescriptor.WAV_MIME_TYPE) &&
					MediaDescriptorUtility.isVideoType(md)) {
				if (FileUtility.sameNameIgnoreExtension(md.mediaURL,
							otherMD.mediaURL)) {
	...
	/**
	 * Check to prevent the same file from being linked twice.
	 * 
	 * @param url the URL to check
	 * @return {@code true} if there is already a media descriptor with this URL
	 */
	 ...
	 * may have to be changed, e.g. when there is an mediadescriptor with an
	...
	/**
	 * Presents an input pane for the URL of a remote file.
	 * Copied from MultiFileChooser, modified Mar 2020
	 * 
	...
	private String chooseRemoteFile() {
		...
		for (;;) {
	        ...
	        url = (String) rf;
	        url.replace('\\', '/');
	        // perform naive test, don't repair
		...
		for (int i = 0; i < currentMDCopy.size(); i++) {
			...
			if (amd.mimeType.equals(MediaDescriptor.WAV_MIME_TYPE)) {
				for (int j = 0; j < currentMDCopy.size(); j++) {
					...
					if (MediaDescriptorUtility.isVideoType(vmd)) {
