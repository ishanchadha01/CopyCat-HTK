...
/**
 * Viewer for text of a selected tier with highlighting where the cursor is,
 * highlighting of the selection and marking of the active annotation.
 * 
...
//            ex.printStackTrace();
            LOG.warning("Exception while setting up the TextViewer: " + ex.getMessage());
        }
    }

    /**
     * Repaints after a change in selection.
     */
    @Override
    ...
    /**
     * Repaints after a change in active annotation. 
     */
    @Override
    ...
    /**
     * Notification of edits in the document.
     *
     * @param e the event
     */
    @Override
    ...
    /**
     * Changes font (size) and visualization options.
     */
    @Override
    ...
    /**
     * Repaints, in the process of which checks are performed on the media time,
     * selection, active annotation etc.
     * 
    ...
    public void doUpdate() {
        ...
        try {
            ...
            try {
            	...
            	LOG.warning("Error media position highlight: " + ble.getMessage());
            	currentPainter.setVisible(false);
            }
            ...
            try {
            	...
            	LOG.warning("Error active annotation highlight: " + ble.getMessage());
            	activeAnnotationPainter.setVisible(false);
            }
            ...
            try {
            	...
            	LOG.warning("Error interval selection highlight: " + ble.getMessage());
            	selectionPainter.setVisible(false);
            }
        } catch (Exception ex) {
            //ex.printStackTrace();
        	LOG.warning("Update error: " + ex.getMessage());
        }
        				...
        				while (j < arrTagPositions.length - 1) {
        					if (arrTagPositions[j] == pos) {
        						try {
        							...
        							LOG.warning("Error CV values highlight: " + ble.getMessage());
        						}
        						break;
        					}
        					j++;
        				...
        				while (j < arrTagPositions.length - 1) {
        					if (arrTagPositions[j] == pos) {
        						try {
        							...
        							LOG.warning("Error CV values highlight after changing the tier: " + ble.getMessage());
        						}
        						break;
        					}
        					j++;
    ...
    /**
     * Gets the tier which is shown in the text viewer.
     *
	 ...
	 * 
	 * @return {@code true} if the annotation values are visualized separated by a dot, 
	 ...
	 * 
	 * @param dotted when {@code true} the annotations are separated by dots
	 */
	 ...
	 * will most of the time be drawn at the bottom of the view.
 	 *   
	 * @param centered if {@code true} the view is centered vertically
	 */
    ...
    /**
     * Builds an array with all begin times and end times from the annotations
     * (tags).
     * Used for searching (quickly) a particular tag.
     */
...
//        }
    ...
    /**
     * Component listener, handles resize events.
     */
