...
/**
 * A recognizer which creates a silence/non-silence segmentation for a wave
 * file. It does not distinguish between speech and non-speech sounds.
 * 
 * @author albertr
 * @author aarsom, Sep 2012
 *
 */
public class SilenceRecognizer implements Recognizer {
	...
	private WAVSamplesProvider sampler;
	...
	/**
	 * Called by RecognizerHost to get a name for this recognizer in the 
	 * ComboBox with available recognizers.
	 * 
	 ...
	 * 
	 * @return a JPanel with the recognizers GUI controls
	 */
	@Override
	...
	private void getSamples(long from, long to, int channel, int[] samples) {
		try {
			...
			if (channel == 1) {
				System.arraycopy(sampler.getChannelArray(0), srcPos, samples, 0, length);
			} else {
				System.arraycopy(sampler.getChannelArray(1), srcPos, samples, 0, length);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
	 * Retrieves the maximum value from an array of samples.
	 * 
	...
	/**
	 * Returns the number of zero-crossings in the samples array.
	 * 
	 * @param samples the samples to test
	 * @return the count of zero-crossings
	 */
	...
	/**
	 * Calculates the average energy of an array of samples.
	 * 
	...
	/**
	 * Called by RecognizerHost to stop the recognizer.
	 *
	 */
	@Override
	public void stop() {
	 ...
	 * The actual work.
	 * 
	 * Progress: 10% threshold calculation, 80% stepping frames, 
	 * calculating averages, 10% pruning and creating segmentations
	 */
	public void recog() {
		...
		try {
			...
			nrOfChannels = sampler.getNumberOfChannels();
...
//		}
	...
	public boolean canHandleMedia(String mediaFilePath) {
		...
		try {
			WAVSamplesProvider wavs = new WAVSampler(mediaFilePath);
			int nc = wavs.getNumberOfChannels();
			if (nc == 0) {
				return false;
			}
	...
	/**
	 * For text and file parameters, empty implementation.
	 */
	@Override
	...
	public Object getParameterValue(String param) {
		...
		if (param.equals("threshold_1")) {
			return Float.valueOf(noiseThreshold1);
		} else if (param.equals("threshold_2")) {
			return Float.valueOf(noiseThreshold2);
		} else if (param.equals("silence_dur")) {
			return Float.valueOf(silenceDur);
		} else if (param.equals("non_silence_dur")) {
			return Float.valueOf(nonSilenceDur);
		}
