...
public class CreateTranscriptionsCommand implements Command, ProcessReporter {
	...
	private void createTranscriptions(File source, String templatePath, String destinationPath, 
		...
		if (files.length == 0) {
			LOG.warning("There are no media files in the source folder");
			return;
		}
		...
		for (File f : fileList) {
			...
			if (index > -1) {
				...
				if ( !waveList.contains(f) ) {
					...
					MediaDescriptor md1 = MediaDescriptorUtility.createMediaDescriptor(path);
					mds.add(md1);
					MediaDescriptor md = null;
					if (wavFile != null) {
						processedFiles.add(wavFile);
						md = MediaDescriptorUtility.createMediaDescriptor(wavFile.getAbsolutePath());
					...
					if (otherVideos != null) {
						for (File ov : otherVideos) {
							if (ov != null) {
								md = MediaDescriptorUtility.createMediaDescriptor(ov.getAbsolutePath());
								mds.add(md);
							}
						}
					}
					...
					MediaDescriptor md = MediaDescriptorUtility.createMediaDescriptor(f.getAbsolutePath());
					mds.add(md);
					trans.setMediaDescriptors(mds);
				}
			} else {
				...
				MediaDescriptor md = MediaDescriptorUtility.createMediaDescriptor(path);
				List<MediaDescriptor> mds = new ArrayList<MediaDescriptor>(1);
				mds.add(md);
				trans.setMediaDescriptors(mds);
			}
				...
				try {
					...
					if (templatePrefsPath != null) {
						...
						try {
							...
							LOG.warning("Cannot copy the preferences file: " + nextPfsxPath + " : " + t.getMessage());
						}
					}
					...
					LOG.warning("Cannot save a new transcription file: " + nextEafPath);
				}
			} else {
				LOG.warning("Cannot save a new transcription for file: " + path);
			}
		}
	}
