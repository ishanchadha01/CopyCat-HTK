...
outerloop: 
    ...
    private static Map<String, String> checkLinkedFiles(TranscriptionImpl transcription) {
        ...
        for (int i = 0; i < lfDescs.size(); i++) {
            ...
            // media files (audio and video) have been checked in a "default" location 
            // in ElanFrame2, check other files here if there is a default alternative location
            String fallBackLocation = Preferences.getString("DefaultGeneralLinkedFilesLocation", null);
            if (fallBackLocation != null) {
            	String resolvedURL = fallBackResolve(fallBackLocation, linkName);
            	// if !null update the URL
            	if (resolvedURL != null) {           	
	                lfd.linkURL = FileUtility.pathToURLString(resolvedURL);
	                LOG.info("Updating url from: " + oldLinkURL + " to: " +
	                    lfd.linkURL);
	                //transcription.setChanged();// don't set the changed flag in this case
	                linkMap.put(oldLinkURL, lfd.linkURL);
    ...
    /**
     * Tries to find a file in a selected folder or sub-folder.
     * 
     * @param fallBackLocation the folder selected by the user
     * @param fileName the file to find
     * @return the full path of the file pr {@code null}
     */
    private static String fallBackResolve(String fallBackLocation, String fileName) {
    	// remote default location is not actually supported at the moment
    	...
    	// the folder exist, check the file
    	File f = new File(localFolder, fileName);
    	if (f.exists()) {
    		return f.getAbsolutePath();
    	}
    	// search sub-folders
    	return findRecursive(localFolder.listFiles(), fileName);
    }
    		...
    		for (File f : fileList) {
    			if (f.isDirectory()) {
	    			...
	    			if (cf.exists()) {
	    				...
	    				String recFile = findRecursive(f.listFiles(), fileName);
	    				if (recFile != null) {
	    					return recFile;
	    				}
	    			}
