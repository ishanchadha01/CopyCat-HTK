...
public class AnnotationValuesToTiersCommand implements UndoableCommand {
	...
	public void execute(Object receiver, Object[] arguments) {
		...
		if (sourceTier == null) {
			LOG.warning("There is no tier with that name: " + sourceTierName);
			return;
		}
		maxNumTiers = -1;
		...
		if (segmentsPerTier == null) {
			LOG.warning("No new tiers have been extracted from the input tier: " + sourceTierName);
			return;
		}
	...
	private void createTiers() {
		if(segmentsPerTier == null) {
			LOG.warning("No tiers to create.");
		}
		...
		while (tierIter.hasNext()) {
			if (count == 0) {
				nextType = createType();
				if (nextType == null) {
					LOG.warning("Unable to create a linguistic type, cannot create new tiers.");
					break;
				}
			}
			
			if (maxNumTiers > 0 && count >= maxNumTiers) {
				LOG.warning("The maximum number of tiers has been reached, stopping here.");
				break;
			}
			...
			if (nextTier == null) {
				LOG.warning("Could not create a tier for: " + tierName);
				continue;
			}
		...
		if (transcription != null && tierNameMap != null) {
			...
			while (tierIter.hasNext()) {
				...
				if (tier != null) {
					...
					LOG.warning("The  tier " + tierName + " does not exist and can therefore not be deleted.");
				}
			}
