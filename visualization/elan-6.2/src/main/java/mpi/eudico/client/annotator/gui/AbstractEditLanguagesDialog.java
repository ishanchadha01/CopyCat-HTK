...
public abstract class AbstractEditLanguagesDialog extends JDialog
	 ...
	 * 
	 * @param id the id to get the index for
    ...
    /**
     * A combobox editor for languages.
     */
    public static class ComboBoxLanguageEditor
		 ...
		 * Constructor.
		 * 
		 * @param defaultChoice the default language to select
		 */
    	 ...
    	 * 
    	 * @param validListener the action listener
    	 */
		...
		/**
		 * Return a {@code LangInfo} from the text presently in the text fields.
		 * Checks it for uniqueness.
		 * Returns null if not good enough.
		 * 
		 * @return the selected or created {@code LangInfo} or {@code null}
		 */
    	@Override
    	 ...
    	 * @param li the {@code LangInfo} to check
    	 * @return given LangInfo if ok, or {@code null} otherwise.
    	 */
    	private LangInfo checkIfValid(LangInfo li) {
    		...
    		if (edited) {
    			if (s.length() < 2) {// now allow 2 letter id's
    				return null;
    			}
    			idMatcher.reset(s);
