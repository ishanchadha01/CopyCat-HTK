...
/**
 * Manager for the Segmentation mode, optimized for on-the-fly segmentation
 * by means of one or two key strokes.
 * 
 */
public class SegmentationManager implements ModeLayoutManager {
	...
	/**
	 * Creates and adds media players, viewers and the segmentation control panel.
	 */
	@Override
	...
	/**
     * Sets the media player controller. 
     * In this mode the controller also contains a step-and-repeat control panel.
     *
    ...
    /**
     * Lays out the components depending on dimensions and preferences.
     */
    @Override
	public void doLayout() {
		...
		if (oneRowForVisuals) {
			...
			if (numVisualPlayers == 2) {
				...
				float firstAR =  visualPlayers[0].player.getAspectRatio();
				if (firstAR == 0) {
					firstAR = 1.0f;
				}
				...
				if (firstMediaWidth + secondMediaWidth + componentMargin > maxWidthForMedia) {
					...
					firstMediaHeight = (int) (firstMediaWidth / firstAR);
	...
	/**
	 * Removes components and viewers from the layout and the container.
	 */
	@Override
	...
	/**
	 * Creates and adds relevant components to the container.
	 */
	@Override
	...
	/**
	 * Updates the layout based on preference settings.
	 */
	@Override
	...
	/**
	 * Enables or disables menu items that apply to this mode.
	 */
	@Override
	 ...
	 *  haven't been stored yet and also stores any 
	...
	/**
	 * 
	 * @return a list of zoomable viewers, first the segmentation viewer, then the signal viewer
	 */
	@Override
