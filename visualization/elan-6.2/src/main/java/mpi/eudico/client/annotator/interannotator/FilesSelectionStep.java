...
implements ChangeListener {
	...
	protected void initExportPanel() {
		...
		exportCombineTiersCB = new JCheckBox(ElanLocale.getString(
				"CompareAnnotatorsDialog.FilesSelectionStep.SaveMatchingTiers"));
		exportFilePerSetOfTiersRB = new JRadioButton(ElanLocale.getString(
				"CompareAnnotatorsDialog.FilesSelectionStep.SaveFilePerTierGroup"), true);
		exportFilePerSetOfFilesRB = new JRadioButton(ElanLocale.getString(
				"CompareAnnotatorsDialog.FilesSelectionStep.SaveFilePerFileGroup"));
		...
		if (exportTierPanel.isVisible()) {
			if (exportCombineTiersCB.isSelected()) {
				...
				if (exportFilePerSetOfTiersRB.isSelected()) {
					multiPane.putStepProperty(CompareConstants.EXPORT_TIERS_METHOD_KEY, CompareConstants.EXPORT_TIERS_PERTIERSET);
				} else {
					multiPane.putStepProperty(CompareConstants.EXPORT_TIERS_METHOD_KEY, CompareConstants.EXPORT_TIERS_PERFILE);
				}
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == exportFolderBrowseBT) {
				...
				chooser.createAndShowFileDialog(ElanLocale.getString("Button.Browse"), 
						FileChooser.OPEN_DIALOG, ElanLocale.getString("Button.Select"), 
