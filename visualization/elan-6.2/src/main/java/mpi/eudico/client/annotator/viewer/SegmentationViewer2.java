...
MultiTierViewer, AdjustmentListener, ACMEditListener, ChangeListener {
    ...
    /**
     * Creates a new SegmentationViewer2 instance.
     */
...
//        	editTier2d = allTiers.get(0);
    ...
    private Tier2D createTier2D(TierImpl tier) {
        ...
        for (Annotation a : tier.getAnnotations()) {
            ...
            int xPos = timeToPixels(a.getBeginTimeBoundary());
            tag2d.setX(xPos);
            int tagWidth = timeToPixels(a.getEndTimeBoundary()) - xPos;
    ...
    private void recalculateTagSizes() {
        ...
        while (tierIt.hasNext()) {
            ...
            while (tagIt.hasNext()) {
                ...
                int xPos = timeToPixels(tag2d.getBeginTime());
                tag2d.setX(xPos);
                int tagWidth = timeToPixels(tag2d.getEndTime()) - xPos;
                tag2d.setWidth(tagWidth);
	...
	/**
     * Override <code>JComponent</code>'s paintComponent to paint ruler, tags
     * and cursor.
     * It first paints a BufferedImage with a ruler and the tags, then the 
     * cursor / crosshair on top of it.
     *
...
//        }       
	...
	/**
     * Sets the control panel for this viewer.
     *
     ...
     * 
     * @param cve the CV entry, can be {@code null}
     */
...
//        }
	...
	protected void setLocalTimeScaleMsPerPixel(float step) {
        ...
        setPreference("SegmentationViewer.ZoomLevel", Float.valueOf(zoom), 
        		transcription);
	 ...
	 * 
     * @see mpi.eudico.client.annotator.viewer.AbstractViewer#controllerUpdate(mpi.eudico.client.annotator.ControllerEvent)
     */
    @Override
...
//            System.out.println("IE time: " + intervalEndTime);
	...
	public void ACMEdited(ACMEditEvent e) {
        ...
        case ACMEditEvent.ADD_ANNOTATION_HERE:
            ...
            if (e.getInvalidatedObject() instanceof TierImpl &&
            	...
            	// check if a cv entry value needs to be added to the new annotation?
            	if (lastRecord != null) {
	...
	private void createAnnotation(TimeInterval ti) {
		if (ti != null && editTier2d != null) {
	        ...
	        Object[] args = new Object[] {
	                Long.valueOf(ti.getBeginTime()), Long.valueOf(ti.getEndTime())
	            };
	...
	public void setNextTierActive(boolean down) {
		...
		for (int i = 0; i < visibleTiers.size(); i++) {
			Tier2D nt2d = visibleTiers.get(i);
...
//        } 
	...
	private Tag2D getAnnotationAtX(int x) {
		...
		for (int i = 0; i < editTier2d.getTagsList().size(); i++) {
			Tag2D t2d = editTier2d.getTagsList().get(i);
...
//			System.out.println("Segment " + (up ? "Up" : "Down") + "   Is key down: " + pressReleaseKeyDown);
    ...
    public class SegmentAction extends AbstractAction {
        ...
        /**
         * Constructor with a controlled vocabulary as parameter.
         * 
         * @param cve a CV entry
         */
...
//        	System.out.println("Segment action");
	...
	public class MergeAnnotationAction extends AbstractAction {
		 ...
		 * 				else merge with annotation before
		 */
