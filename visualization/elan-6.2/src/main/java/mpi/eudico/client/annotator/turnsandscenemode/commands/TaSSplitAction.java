...
/**
 * An action that initiates a {@code Split} action if a specific keystroke occurred.
		...
		if (curAnno.getAnnotation() != null) {
			...
			if (!shouldSplit || (shouldSplit && annHasDepending)) {// do the same in both cases, modify the text
				if (shouldSplit && annHasDepending) {
					LOG.warning("Splitting of annotations that have depending annotations is not supported.");
