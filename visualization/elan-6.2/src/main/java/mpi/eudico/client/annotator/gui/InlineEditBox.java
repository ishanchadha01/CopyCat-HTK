...
//    private final static int EDIT_INTERUPPTED = 2;
    ...
    /**
     * Performs initialization incorporating user preferences.
     */
    public void init() {
...
//					}			
    ...
    /**
     * Adds a {@code InlineEditBoxListener}.
     * 
     * @param inLineListener the new inline edit box listener
     */
    ...
    /**
     * Removes the {@code InlineEditBoxListener}.
     * 
     * @param inLineListener the listener to remove
     */
     ...
     * 
     * @param edit_Type the type of the change
     */
...
//    			break;
     ...
     * 
     * @param textAreaToCheck the text component to spell check
     */
    ...
    private Dimension getCVCompSize(Dimension size){
    	...
    	minCVHeight = cvEntryComp.getEditorComponent().getFontMetrics(
    			cvEntryComp.getEditorComponent().getFont()).getHeight();
...
//        	inlineBoxWidth = cvw;
     ...
     *
     * @param preferredComponent the class of the component to configure
     * @param font the font to be used by the editor
     * @param size the size available for the editor
     *
...
//        }
    ...
    /**
     * Conditionally restores original value and caret position.
     *
     * @param e the key event
     */
    @Override
    ...
    class CVEntryComponent implements KeyListener, ActionListener, DocumentListener {
       ...
       /**
         ...
         * committing and canceling need to be provided.  Applies only to
         * the JList component.
         */
        ...
        private void initComponents(Class<?> component) {
        	if(showCVDescription){
        		...
        		if(component == JPanel.class){
    				...
    				textField.setFont(InlineEditBox.this.getFont());
        		...
        		if (component == JScrollPane.class) {
                    if (entryList == null) {
                        entryListModel = new DefaultListModel<SimpleCVEntry>();
                        entryList = new JList<SimpleCVEntry>(entryListModel);
    						...
    						public void mouseMoved(MouseEvent e) {
    			                ...
    			                if ((row > -1 && row < entryListModel.size())) {
    			                	if(row != lastRow) { // The user changed rows
    			                		...
    			                		String cvEntryId = (entryListModel.getElementAt(row)).getId();
    			                		InlineEditBox.this.showVideoForEntry(cvEntryId);
    			                	}
    			                }
    						}
                    ...
                    if (box == null) {
                        entryBoxModel = new DefaultComboBoxModel<SimpleCVEntry>();
                        box = new JComboBox<SimpleCVEntry>(entryBoxModel);
                        box.setFont(InlineEditBox.this.getFont());
        			...
        			if (suggestEntryList == null) {
        				suggestEntryListModel = new DefaultListModel<SimpleCVEntry>();
        				suggestEntryList = new JList<SimpleCVEntry>(suggestEntryListModel);
        ...
        public void setFont(Font f) {
        	//delegate.setFont(f);// the delegate is used in some calculations
            if (delegate == box) {
                box.setFont(f);
            	...
            	textField.setFont(f);
            	// the delegate is used in some dimension calculations
            	suggestPanel.setFont(f);
            	if(showCVDescription){
            	...
            	if (reload) {
            		...
            		if (cveId != null) {
            			for (int i = 0; i < entryListModel.size(); i++) {
            				SimpleCVEntry entry = entryListModel.getElementAt(i);
    	                	String id = entry.getId();
            				if (cveId.equals(id)) {
        ...
        public SimpleCVEntry getSelectedEntry() {
            ...
            if (delegate == scrollPane) {
                if (entryList.getSelectedValue() != null) {
                    value = entryList.getSelectedValue();
                }
            } else if (delegate == box) {
            	...
            	if(showCVDescription){
    				...
    				value = suggestEntryList.getSelectedValue();
    			} 
    		} else if (delegate == tableScrollPane){
    			if(entryTable.getSelectedRow() > -1){
        			value = (SimpleCVEntry)entryTable.getValueAt(entryTable.getSelectedRow() , 0);
    ...
    /**
     * Tries to display a video connected to the entry.
     * 
     * @author Micha Hulsbosch
     * @param cvEntryId the entry id
     */
