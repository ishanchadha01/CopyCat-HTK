...
/**
 * The Command Factory is the central point for command constants, for creating
 * and caching commands and command actions and for the administration of the
...
public class ELANCommandFactory {
	...
	public static CommandAction getCommandAction(Transcription tr, String caName) {
		...
		if (ca == null) {
			...
			//temp
			...
			else if (caName.equals(EXPORT_TA_INTERLINEAR_GLOSS)) {
				...
				ca = new ExportHTMLDlgCA(viewerManager);
                        ...
                        else if (caName.equals(EXPORT_JSON)) {
			...
			else if (caName.equals(SET_DOCUMENT_PROPERTIES)) {
				ca = new SetDocumentPropertiesDlgCA(viewerManager);
			}
			else if (caName.equals(DOCUMENT_INFO)) {
				ca = new DocumentInfoCA(viewerManager);
			}
			else if (caName.equals(ANNOTATION_DENSITY_PLOT)) {
				ca = new AnnotationDensityPlotCA(viewerManager);
			}
			else if (caName.equals(AUDIO_SPECTROGRAM)) {
	...
	public static Command createCommand(Transcription tr, String cName) {
		...
		// temp
		...
		else if (cName.equals(EXPORT_TA_INTERLINEAR_GLOSS)) {
			...
			c = new ExportHTMLDlgCommand(cName);
                ...
                else if (cName.equals(EXPORT_JSON)) {
		...
		else if (cName.equals(SET_DOCUMENT_PROPERTIES)) {
			c = new SetDocumentPropertiesCommand(cName);
		}
		else if (cName.equals(SET_DOCUMENT_PROPERTIES_DLG)) {
			c = new SetDocumentPropertiesDlgCommand(cName);
		}
		else if (cName.equals(DOCUMENT_INFO)) {
			c  = new DocumentInfoCommand(cName);
		}
		else if (cName.equals(ANNOTATION_DENSITY_PLOT)) {
			c = new AnnotationDensityPlotCommand(cName);
		}
		else if (cName.equals(AUDIO_SPECTROGRAM)) {
	...
	/**
	 * Returns the undo command action. This is the action which is added to the 
	 * Edit menu and which calls the command history's {@code undo}.   
	 *
	...
	/**
	 * Returns the redo command action. This is the action which is added to the 
	 * Edit menu and which calls the command history's {@code redo}.   
	 *
