...
/**
 * Utility class that can (1) be used to detect a common prefix or suffix of
 * two tier names (2) update tier names in AnnotationDataRecords.
 * The latter is based on a given delimiter and left or right correspondence 
 * (part left of the delimiter is the same, or part right of the delimiter).  
 * 
 * @author Han Sloetjes
 */
public class TierNameCompare {
     ...
     * @return an array of size 2: index 0 is the index of a delimiter character
     * in the first name, index 1 determines whether the begin (part before the 
     * delimiter) is the same ({@code PREFIX_MODE}), or the end 
     * ({@code SUFFIX_MODE}) (part after the delimiter)
     */
     ...
     * @param leftOrRight if {@code <= KEEP_LEFT_PART}, the part left of the last "from" character should be maintained, 
     ...
     * @param root the root of the annotation tree
     * @param newAffix the new prefix or suffix
     * @param leftOrRight if {@code <= KEEP_LEFT_PART}, the affix is a suffix, otherwise it is a prefix
     */
