...
/**
 * Variant of the {@link SignalViewer} with a direct link to a media player,
 * for use in the synchronization mode.
...
public class SignalPlayerView extends TimeScaleBasedViewer
    ...
    /** mono mode, a single audio track */
    public static final int MONO = 0;

    /** stereo mode, with two separately visualized tracks */
    public static final int STEREO_SEPARATE = 1;

    /** stereo mode, with two tracks, visually merged into one */
    public static final int STEREO_MERGED = 2;

    /** stereo mode, with two visually superimposed, blended tracks */
    ...
    /**
     * Create a SignalPlayerView with some default values but without any data to
     * display.
     */
    ...
    /**
     * Creates a new SignalPlayerView instance.
     *
     * @param mediaUrl the path to the media file
     */
    ...
    /**
     * Creates a new SignalPlayerView using the specified path as the media source.
     * 
     ...
     * 
     * @param wavPlayer the WAV media player
     */
    ...
    /**
     * Returns whether this player view is connected for synchronization.
     * 
     * @return {@code true} if this player view is currently connected for
     * synchronization
     */
	...
	/**
	 * Sets the synchronization connected state of this player view.
	 * 
	 * @param syncConnected if {@code true} this view is connected for synchronization
	 */
	...
	*/
    ...
    /**
     * Sets the color to use for the selection.
     *
     * @param selectionColor the color for the selection
     */
     ...
     * pixel.
     *
     * @param fromTime the interval start time in ms
     * @param toTime the interval stop time in ms
    ...
    /**
     * Paint to the BufferedImage. This is necessary in the following situations:<ul>
     ...
     *
     * @param big2g the graphics object
     */
    ...
    private void setLocalTimeScaleMsPerPixel(float step) {
        ...
        setPreference("SignalViewer.ZoomLevel", Float.valueOf(zoom), 
     ...
     *
     * @return msPerPixel the milliseconds per pixel value
     */
     ...
     * @param storePref if {@code true} store the mode as a preference
     */
    ...
    /**
     * Returns whether the time ruler is visible.
     * 
	 * @return {@code true} if the time ruler is currently visible
	 */
	 ...
	 * 
	 * @param timeRulerVisible if {@code true} the time ruler is set to visible
	 */
    ...
    /**
     * Returns the time at the start of the current visible interval.
     *
    ...
    /**
     * Returns the time at the end of the current visible interval.
     *
    ...
    private void createPopupMenu() {
        ...
        if ((samp != null) && (samp.getNumberOfChannels() == 2)) {
    ...
    /**
     * Calculates x-coordinates for the begin and end of the selection and
     * repaints. 
     */
    @Override
     ...
     *
     * @return the number of pixels at the left that contain no time line
     *         related data
     */
     ...
     * panel that contains no time line information.
     *
     * @return the number of pixels at the right that contain no time line 
     *         related data
     */
    ...
    /**
     * Calculates a new BufferedImage based on the new size of the component.
     */
    @Override
    ...
    /**
     * Sets the scrolling flag to {@code false}.
     */
    void stopScroll() {
        /*
           if (scroller != null) {
    ...
    /**
     * Scrolls the visible interval in small steps to the left or to the right,
     * as a result of dragging with the mouse.
     */
    class DragScroller extends Thread {
        /** the step size, the number of pixels to scroll */
        int numPixels;

        /** the time between one scroll step and the next */
        long sleepTime;

        /**
         * Creates a new DragScroller instance.
         *
         * @param numPixels the step size
         * @param sleepTime the sleep time between scroll steps
         */
        ...
        /**
         * Calls {@code SignalPlayerView#scroll(int)} at regular intervals.
         */
        @Override
		public void run() {
            while (!stopScrolling) {
