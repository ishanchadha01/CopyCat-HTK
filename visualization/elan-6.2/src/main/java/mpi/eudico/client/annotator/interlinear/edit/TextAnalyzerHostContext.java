...
public class TextAnalyzerHostContext 
	...
	public TextAnalyzerHostContext(InterlinearizationManager manager) {
		...
		configsMap = new HashMap<AnalyzerConfig, List<SourceTargetConfiguration>>(6);
	 ...
	 * <p>
	 * This is called by the {@link InterlinearizationManager} when the stored 
	 * preferences are read and interpreted. This is performed only once now,
	 * nevertheless, this method performs a check to prevent duplicates.
	 *  
	 * @param config the analyzer configuration to add
	 */
	public void addConfig(AnalyzerConfig config) {
		if (!configs.contains(config)) {
			...
			TextAnalyzer ta = TextAnalyzerRegistry.getInstance().getAnalyzerForDocAndConfig(
					transcription.getURN().toString(), config.getAnnotId(), config.getConfigKey());
			...
			if (ta != null) {
				if (loadedAnalyzers.contains(ta)) {
					...
					success = (ta == loadAnalyzer(config, sourceTargetConfList));
				}
			}
			...
			if (success) {
				...
				configsMap.put(config, sourceTargetConfList);
			}
	...
	private Position tierToPosition (Tier tier) {
		...
		if (lexBundle != null) {
			...
			if (lexiconName != null && field != null) {
				pos = new PositionLexicon(tier.getName(), lexiconName, field, tier.getLangRef());
			} else {
	...
	public void removeConfig(AnalyzerConfig config) {
		if (configs.remove(config)) {
			...
			// List<SourceTargetConfiguration> sourceTargetConfList = convertToSTConfList(config);
			List<SourceTargetConfiguration> sourceTargetConfList = configsMap.get(config);
			
			if (sourceTargetConfList == null || sourceTargetConfList.isEmpty()) {
				// log
				if (LOG.isLoggable(Level.INFO)) {
					LOG.log(Level.INFO, String.format(
							"Source-target configuration not removed for analyzer config: %s", config.toString()));
				}
				return;
			}

			unloadAnalyzer(config, sourceTargetConfList);
	...
	/**
	 * 
	 * @param info identifier of the analyzer
	 * @param configKey a configuration key or null
	 * @return a list of all analyzers identified by the {@code info} object 
	 *   and created for the specified configuration, can be null
	 */
	...
	/**
	 * Tests if a specific analyzer is configurable (and therefore provides 
	 * a configuration panel), without triggering the actual creation of the 
	 * panel. 
	 * 
	 * @see #getConfigurationComponent(Information, String, boolean) 
	 * @param info the identifier of the analyzer
	 * @return true if the analyzer has already been loaded and can be 
	 ...
	 * 
	 * @param config the configuration containing the descriptor for the analyzer
     ...
     * @param config the configuration containing the information object of the analyzer
    ...
    private void unloadAnalyzer(AnalyzerConfig config, List<SourceTargetConfiguration> stConfigs) {
        ...
        for (TextAnalyzer a : loadedAnalyzers) {
            ...
            if (config.getAnnotId().equals(a.getInformation())) {
            	toUnLoad = a;
            	break;
            }
        }
        ...
        if (toUnLoad.partUnload(stConfigs) == 0) {
            ...
            TextAnalyzerRegistry.getInstance().removeAnalyzerForDocAndConfig(
            		transcription.getURN().toString(), config.getAnnotId(), config.getConfigKey());
            return;
        }
    ...
    // TODO add a getConfigurationComponent method which accepts an AnalyzerConfig as param?
    /**
     * The configuration components are no longer stored for reuse but always
     * created anew.
     * 
     * @return the configuration component of the first analyzer with the name
...
//				}
	...
	public boolean newAnnotation(Position pos, List<Suggestion> annotations) {
		...
		if (destTier != null) {
			...
			if (cons == null) {
				...
				if (LOG.isLoggable(Level.FINE)) {
					LOG.log(Level.FINE, "The destination tier is an independent tier");
				}
				//return false;
			}
     ...
     * <p>
     * The first call (after starting an interlinearization action)
	...
	public boolean newAnnotations(List<SuggestionSet> sugSets) {
		...
		if (sugSets != null) {
			if (sugSets.size() == 1) {
				...
				analyzeNext();
				return true;
			} else if (sugSets.isEmpty()) {
				// show a message to the user?
				if (LOG.isLoggable(Level.INFO)) {
					LOG.log(Level.INFO, "The analyzer returned an empty list (0 suggestions)");
				}
			} else {
	...
	/**
	 * Called when the user indicates that this suggestion should never be shown again.
	 */
	@Override // SuggestionSelectionListener
	public void suggestionRemoved(SuggestionSelectionEvent event) {
		if (event != null) {
			...
			notifyLexanSuggestionRemoved(sugSet);
		}
	}
