...
public class SharedRecognizer extends LocalRecognizer {
		...
		try {
			if (socket != null) {
				try {
					...
					LOG.warning("Closing socket failed: " + ioe.getMessage());
				}
			}
			...
			LOG.info("Creating socket..." + runCommand);
	...
	private void startHTTP() {
		try {
            ...
            if (respCode != 200) {
    			LOG.severe("Error while accessing server: " + runCommand + " : " + respCode);
            ...
            if (cont instanceof InputStream) {
            	...
            	LOG.severe("Unknown content from server: " + cont);
			...
			LOG.severe("Could not run the recognizer: " + uee.getMessage());
			...
			LOG.severe("Could not run the recognizer: " + mue.getMessage());
			...
			LOG.severe("Could not run the recognizer: " + ioe.getMessage());
	...
	private void readShares() {
		...
		if (SystemReporting.isWindows()) {
			...
			try {
				...
				LOG.warning("Could not read the network shares mappings");
			}
			// test
		...
		public void run() {
			...
			while (isRunning) {
				if (socket != null ) {
					if (socket.isClosed()) {
						isRunning = false;
						LOG.warning("The connection is closed.");
						...
						isRunning = false;
						LOG.warning("The connection is lost.");
				...
				if(idle > reportAfterIdleTime) {
					LOG.warning("Connection idle for: " + TimeFormatter.toSSMSString(idle) + " seconds");
					host.appendToReport("Connection idle for: " + TimeFormatter.toSSMSString(idle) + " seconds\n");
				} else {
					LOG.info("Connection active, last read: " + TimeFormatter.toSSMSString(idle) + " seconds");
				}
