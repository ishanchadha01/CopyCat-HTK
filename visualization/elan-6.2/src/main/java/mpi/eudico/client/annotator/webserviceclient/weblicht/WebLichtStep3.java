	...
	private void fillTokenizerList() {
		...
		if (wlDescList == null) {
			...
			try {
				...
				LOG.warning("Unable to harvest and show relevant WebLicht services: " + ioe.getMessage());
				showWarning(ElanLocale.getString("WebServicesDialog.WebLicht.Warning1") + ": \n" + ioe.getMessage());
				return;
			} catch (Throwable th) {
				LOG.warning("Unable to harvest and show relevant WebLicht services: " + th.getMessage());
				showWarning(ElanLocale.getString("WebServicesDialog.WebLicht.Warning1") + ": \n" + th.getMessage());
				return;
			}
		}
