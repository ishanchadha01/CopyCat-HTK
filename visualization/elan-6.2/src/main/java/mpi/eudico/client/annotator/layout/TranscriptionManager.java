...
/**
 * The layout manager for the Transcription mode, which is dominated by a 
 * table- or spreadsheet-like view on tiers and their annotations.
 *
...
public class TranscriptionManager implements ModeLayoutManager, ActionListener {
     ...
     * 
     * @param object the viewer or controller to add
     */
	@Override
	...
	/**
	 *
	 * @param signalViewer the signal viewer to set
	 */
	...
	/**
     * Installs the media player controller.
     *
     * @param mediaPlayerController the media player controller
     */
    ...
    /**
     * Removes the media player controller. 
     *
     * @param mediaPlayerController the player controller to remove
     */
    ...
    /**
     * Returns whether all components of this mode are initialized.
     * 
     * @return {@code true} if all components are initialized 
	 ...
	 *  @param document the annotation document or {@code null}
	 */
	...
	/**
	 * Applies the new type/size changes to the transcription table.
	 * 
	 * @param hashMap a main tier to dependent tiers mapping
	 * @param hiddenTiers a list of names of tiers to hide from the table
	 * 
	 * @param types a list of tier types for the columns in the table 
	 * @param size the font size for the table
	 * @param valueChanged if {@code true} the table is updated with new column
	 *  types, tiers and hidden tiers
	 */
	private void applyChanges(Map<TierImpl, List<TierImpl>> hashMap, List<String> hiddenTiers, 
			List<String> types, Integer size, boolean valueChanged){				
		if(types == null ){
			transViewer.focusTable();
			return;
		}
	 ...
	 * 
	 * @param enabled if {@code true} the menu's and actions will be enabled,
