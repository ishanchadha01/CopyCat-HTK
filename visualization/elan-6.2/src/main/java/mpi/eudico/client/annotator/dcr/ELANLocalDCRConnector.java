	...
	protected void readDCS() {
        try {
        	...
        	if(cacheRW.getFilePath() == cacheName2){
        		...
        		LOG.warning("No data categories found in the cache...: " + errorString);
        	}
        }
    }
	...
	protected void saveDCS() {
        ...
        if (cacheRW != null) {
            try {
            	...
            	LOG.warning("Could not save the DCR cache: " + pce.getMessage());
            } catch (IOException ioe) {
            	LOG.warning("Could not save the DCR cache: " + ioe.getMessage());
            }
        }
    }
			...
			if(dc.isLoaded()){
				...
				if(diffInDays < INTERVAL_IN_DAYS){
					...
					try {
						...
						LOG.warning("Could not get data category: " + e.getMessage());
						break;
						//e.printStackTrace();
					}
				}
