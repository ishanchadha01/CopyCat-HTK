...
public class TiersSelectionStep extends StepPane implements
    ...
    private void checkConditions() {
    	...
    	groupCompareMode = Boolean.TRUE == multiPane.getStepProperty(CompareConstants.GROUP_COMPARE_KEY);
		...
		if (matchingProp == CompareConstants.MATCHING.MANUAL) {
			...
			if (tierModel instanceof SelectableContentTableModel) {
				...
				if (selectedValues.size() == 0) {
					LOG.warning("For same name or affix based matching at least one tier should be selected in the (first) table.");
					return false;
				}
