...
/**
 * A Color Chooser panel for organizing favorite colors. Saving and loading 
 * favorite color is the responsibility of the caller.
 *  
 * @author Han Sloetjes
 */
...
public class FavoriteColorPanel extends AbstractColorChooserPanel implements ActionListener, 
	...
	/**
	 * Constructor with a resource bundle and an array of favorite colors.
	 * 
	...
	/**
	 * Returns the current array of favorite colors. 
	 * (Does nor make a copy, so allows direct access.)
	 * 
	...
	protected void buildChooser() {
		...
		ImageIcon icon = null;
		try {
			icon = new ImageIcon(this.getClass().getResource("/toolbarButtonGraphics/general/Addii16.gif"));
			if (icon != null) {
	...
	/**
	 * Returns the preferred size, which is based on the size of the array, 
	 * the color cell dimensions and the button panel.
	 * 
	...
	/**
	 * Activates or selects a color cell in the grid. Can be used to insert,
	 * cut, copy or paste etc. a color to/from this cell. 
	 * 
	 ...
	 * 
	 * @return {@code true} if the event's key was one of the four arrow keys
	 */
	@Override
