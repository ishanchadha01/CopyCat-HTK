...
ListSelectionListener, FontSizer {
	...
	private void insertNewAnnotation(TaSAnno tasAnno) {
		if (tasAnno.getAnnotation() == null) {
			...
			LOG.warning("Cannot insert an empty (gap) annotation.");
			return;
		}
		...
		if (overlRows[0] == -1 || overlRows[1] == -1) {
			LOG.warning("Error inserting a new row, there should be at least one "
		...
		if (overlRows[0] == overlRows[1]) {
			...
			if (curRow.getBeginTime() == tasAnno.getBeginTime() && 
				...
				LOG.warning("Cannot replace one single row because the time values "
						+ "of the new annotation are not equal to the existing ones.");
			}
		} else {//error
			LOG.warning("There are more than one overlapping segments on this tier "
	 ...
	 * updated or created.
	 *  
	 * @param annotation the annotation that changed
	 */
	private void updateNeighborSegments(Annotation annotation) {
		if (annotation != null) {
			...
			if (rowIndex == -1) {
				LOG.warning("Error updating neighboring rows, the annotation was not found in the table");
				return;
			}
	...
	/**
	 * Recalculates the height of each row.
	 */
		...
		private void checkRowHeight(int row) {
			if (row > -1) {
				try { // sometimes IllegalArgumentExceptions are thrown for TextHitInfo in TextLayout
					...
					LOG.warning("Cannot update row height: " + t.getMessage());
					//t.printStackTrace();
				}
			}
		}
...
//	@Override
	 ...
	 *  
	 * @param tierName the tier to select
	 */
	 ...
	 * 
	 * @return the current selected tier or {@code null}
	 */
	...
	/**
	 * Returns whether the player should continue to play when a new segment 
	 * is created.
	 * 
	...
	/**
	 * Sets whether the player should continue to play when a new segment 
	 * is created.
	 * 
