...
public class UndoableTranscriptionMerger {
	...
	public void mergeWith(TranscriptionImpl first, TranscriptionImpl second, 
		...
		if (sortedTierObj == null || sortedTierObj.size() == 0) {
			...
			LOG.warning("The requested tiers are not in the transcription.");
			return;
		}
		...
		if (addEverything) {
			...
			for (int i = 0; i < sortedTierObj.size(); i++) {
				...
				if (transA.getTierWithId(tierName) == null) {
					LOG.info("Adding a tier in a second iteration: " + tierName);
					// add new tier...
					if (tier.getParentTier() == null) {
						addNewTopLevelTier(tier, sortedTierObj);
					} else {
	...
	public void undo() {		
		...
		while (modIter.hasNext()) {
			...
			if (recreatedModAnn == null) {
				LOG.warning("Could not recreate a modified annotation: " + modAnn.getTier().getName() + 
		...
		while (delIter.hasNext()) {
			...
			if (recreatedAnn == null) {
				LOG.warning("Could not recreate a deleted annotation: " + deletedAnn.getTier().getName() + 
	...
	public void redo() {
		...
		while (annIter.hasNext()) {
			...
			// check and log
			if (nextAa == null) {
				LOG.warning("Could not create the annotation again in redo: " + aa.getTier().getName() + 
...
//		String origCVName = origType.getControlledVocabularyName();
	...
	private void addAnnotations(TierImpl sourceTier, List<String> tierToInclude) {
		...
		if (destTier == null) {
			LOG.warning("The tier to add annotations to is not found in the target transcription.");
			return; // log
		}
...
//			TierImpl destParentTier = destTier.getParentTier();
			...
			for (AbstractAnnotation srcAnn : sourceAnnotations) {
				...
				if (nextAnn != null) {
					...
					//LOG.warning("Could not create annotation");
				//}
			}
