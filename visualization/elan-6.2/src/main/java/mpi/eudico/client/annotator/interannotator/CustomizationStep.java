...
import mpi.eudico.client.annotator.interannotator.CompareConstants.METHOD;
...
public class CustomizationStep extends StepPane {
    ...
    private void updatePanel(CompareConstants.METHOD method) {
    	...
    	if (method == CompareConstants.METHOD.MOD_KAPPA || 
    			method == CompareConstants.METHOD.MOD_FLEISS) {
    		...
    		if (method == CompareConstants.METHOD.MOD_KAPPA) {
    		...
    		} else if (method == CompareConstants.METHOD.MOD_FLEISS) {
    			percentageLabel.setText(ElanLocale.getString("CompareAnnotatorsDialog.CustomizeStep.PercentageLabelAverage"));
    			verboseOutCB.setText(ElanLocale.getString("CompareAnnotatorsDialog.CustomizeStep.VerboseOutputValues"));
    			
    			// add an option to create tables for all tiers sharing the same CV ??
    			overlapSlider.setMinimum(1);
        		Dictionary<Integer, JComponent> labels = new Hashtable<Integer, JComponent>(10);
        		...
        		overlapSlider.setLabelTable(labels); // requires a Dictionary (Hashtable)
        		// read preferences
        		Integer sliderPref = Preferences.getInt(CompareConstants.OVERLAP_AVERAGE, null);
        		if (sliderPref != null) {
        			int percentage = sliderPref;
        		...
        		Boolean verbosePref = Preferences.getBool(CompareConstants.OUTPUT_TABLES_VALUES, null);
        		if (verbosePref != null) {
        			verboseOutCB.setSelected((Boolean) verbosePref);
        		}
    		}
	...
	public boolean leaveStepForward() {
		...
		} else if (curMethod == CompareConstants.METHOD.MOD_FLEISS) {
			if (overlapSlider != null) {
				multiPane.putStepProperty(CompareConstants.OVERLAP_AVERAGE, Integer.valueOf(overlapSlider.getValue()));
				Preferences.set(CompareConstants.OVERLAP_AVERAGE, Integer.valueOf(overlapSlider.getValue()), null);
			}
			if (verboseOutCB != null) {
				multiPane.putStepProperty(CompareConstants.OUTPUT_TABLES_VALUES, Boolean.valueOf(verboseOutCB.isSelected()));
				Preferences.set(CompareConstants.OUTPUT_TABLES_VALUES, Boolean.valueOf(verboseOutCB.isSelected()), null);
			}
