 ...
 * (deleted) annotations.
 * It can: <ul>
 * <li> create a dependency tree for annotations and store relevant data from
 * each annotation involved
 * <li> recreate annotations (including dependent annotations) given the stored
...
public class AnnotationRecreator {
    ...
    /**
     * Recreates annotations on Tiers with the specified LinguisticType.
    ...
    /**
     * Recreates annotations on the specified Tier.
     * This operation can be necessary when an other kind of Annotation type
     * is needed after a modification in the type, e.g. conversion of
     * AlignableAnnotations  into SVGAlignableAnnotations when the
     * LinguisticType has been changed  to allow graphic references.<br>
    ...
    /**
     * Creates a tree structure from one annotation.
...
uploop: 
    ...
    /**
     * Creates a treenode without children from one annotation.
     *
     * @param aa the annotation
     *
   ...
   /**
     ...
     * information  contained in the specified Node.
     ...
     * 
     * @param trans the target transcription
     * @param root the root node of the annotation tree
     * @param includeID if {@code true} the annotation id is (or id's are) 
     * applied to the new annotation(s)
     * @param tierMapping if not {@code null}, the target tiers are found on
     * the basis of this mapping of tier names  
     * 
     * @return the created annotation or {@code null}
     */
    public static AbstractAnnotation createAnnotationFromTree(
	...
	/**
	 * This variant does not recreate the id's of annotations.
	 * 
	 * @param trans the Transcription
	 * @param annotationsNodes a List containing a DefaultMutableTreeNode
	 *        for each annotation to recreate
	 * 
	 * @see #createAnnotationsSequentially(Transcription, List, boolean)
	 */
    ...
    /**
     * Creates a number of annotations with child-annotations in a sequence. 
     ...
     *        for each annotation to recreate
     * @param includeId if {@code true} the id's of annotations are recreated too
     *
