 ...
 * @since August 2013
 */
public class ExportNgramStatistics extends AbstractBasicExportDialog {
     ...
     * @param res data structure containing ngram statistics to export
     */
	public ExportNgramStatistics(Frame parent, boolean modal, Transcription transcription, NgramStatsResult res) {
	...
	protected boolean startExport() throws IOException {
		...
		writer.write("# Export of N-gram Analysis done on " + new Date().toString() + NEWLINE);
		...
		writer.write("# Total Collated N-grams: " + result.getNumCollectedNgrams() + NEWLINE);
		writer.write("#" + NEWLINE);

		// write out the columns
		List<String> columns = result.getCollectedNgramAt(0).toCSVColumns();
		...
		for (int i=0; i < result.getNumCollectedNgrams(); i++) {
			writer.write(result.getCollectedNgramAt(i).toCSV(DELIM) + NEWLINE);
		}
		writer.close();
