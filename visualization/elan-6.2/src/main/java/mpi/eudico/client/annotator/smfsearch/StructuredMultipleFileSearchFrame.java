...
public class StructuredMultipleFileSearchFrame extends ClosableFrame
    ...
    public StructuredMultipleFileSearchFrame(ElanFrame2 elanFrame) {
        ...
        if (curDomain == null) {
            //initialize the frame with an empty domain
        	if (LOG.isLoggable(Level.INFO)) {
        		LOG.log(Level.INFO, "No files stored or selected for search, opening frame with empty domain");	
        	}
        	curDomain = new ArrayList<File> (0);
        }
	...
	public ArrayList<File> getDomain() {
        ...
        if (searchFiles.length == 0) {
        	if (LOG.isLoggable(Level.INFO)) {
        		LOG.log(Level.INFO, "The list of selected files to search is empty");	
        	}
            return null;
        }
        ...
        if (domain.size() == 0) {
        	if (LOG.isLoggable(Level.INFO)) {
        		LOG.log(Level.INFO, "The selected search domain is empty");	
        	}
        	return null;
        }
