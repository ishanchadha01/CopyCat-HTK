...
import mpi.har.FeatureViewer;
import mpi.har.AnnotationViewer;
import mpi.har.PipelineGUI;
...
public class ElanFrame2 extends JFrame implements ActionListener,
     ...
     *
     * @param eafPath the path to the EAF file
     * @param mediaFiles a list of media file paths
     */
     ...
     * @param mediaFiles a list containing associated media files
     */
    ...
    public boolean checkMedia(Transcription transcription, String eafFolderPath) {
        ...
        try {
            ...
            for (int i = 0; i < mediaDescriptors.size(); i++) {
                ...
                /* this is now covered by the utility method isRemoteFile()
                if (colonPos > 0) {
                ...
                if (!file.exists()) {
                    ...
                    // check remote location if the eaf file is a remote file and the media file
                    // a local file (which could not be found)
                    if (FileUtility.isRemoteFile(fullEAFURL)) {
                    	...
                    	// check same directory path
                    	String mediaUrl = eafFolderPath +"/" + localFileName;
                    	// check if the remote file exists?
                    	if (FileUtility.remoteFileExists(mediaUrl)) {
                            // adjust urls
                            adjustMediaDescriptors(mediaDescriptors, i,
                            		mediaUrl);
                            LOG.info("Updated file location from: \"" + fileName + 
   ...
   private boolean linkingTheSameMediaTwice(List<MediaDescriptor> mediaDescriptors, String absolutePath) {
    ...
    private void adjustMediaDescriptors(List<MediaDescriptor> mediaDescriptors, int index,
        ...
        mediaDescriptors.get(index).mimeType = MediaDescriptorUtility.mimeTypeForExtension(newExt);
...
//        menuOptions.add(menuItemRateVol);
        ...
        menuHAR = new JMenu("HAR");
        JMenuItem featureViewer = new JMenuItem(new AbstractAction("Feature Viewer") {
            public void actionPerformed(ActionEvent e) {
                FeatureViewer fv = new FeatureViewer();
            }
        });
        JMenuItem annotationViewer = new JMenuItem(new AbstractAction("Annotation Viewer") {
            public void actionPerformed(ActionEvent e) {
                AnnotationViewer av = new AnnotationViewer();
            }
        });
        JMenuItem pipelineGui = new JMenuItem(new AbstractAction("Pipeline GUI") {
            public void actionPerformed(ActionEvent e) {
                PipelineGUI pg = new PipelineGUI();
            }
        });
        menuHAR.add(annotationViewer);
        menuHAR.add(featureViewer);
        menuHAR.add(pipelineGui);
        menuBar.add(menuHAR);

        for (int i=0; i<menuBar.getMenuCount(); i++) System.out.println(menuBar.getMenu(i).getText());
...
//        }     
        ...
        menuExport.add(new ElanMenuItem(ELANCommandFactory.getCommandAction(
            transcriptionForThisFrame,
            ELANCommandFactory.EXPORT_TA_INTERLINEAR_GLOSS)));
        menuExport.add(new ElanMenuItem(ELANCommandFactory.getCommandAction(
...
//        menuItemRateVol.setEnabled(true);
    ...
    /**
     * Notification to update locale dependent labels and menus.
     */
    @Override
