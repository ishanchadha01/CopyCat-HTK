    ...
    private void loadPreferences() {
		...
		if (transcription == null) {
		    ...
		    if (prefTierSource != null) {
		    	if (CompareConstants.FILE_MATCHING.ACROSS_FILES.value.equals(prefTierSource)) {
	...
	public boolean leaveStepForward() {
		if (transcription != null) {
			...
			if (currentDocumentRB.isSelected()) {// error condition, shouldn't happen
				LOG.warning("Cannot proceed to the next step when \"current\" is selected and there's no transcription loaded.");
				return false;
			}
