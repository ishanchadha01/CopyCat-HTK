...
/**
 * A panel which allows the user to either select a tier or create manual 
 * selections or specify a file containing selections.
 * 
...
public class TierSelectionPanel extends AbstractSelectionPanel implements ActionListener {
	...
	/**
	 * Constructor with the file selection option disabled.
	 * 
	 * @param mode either {@code AbstractSelectionPanel#AUDIO_MODE} or {@code AbstractSelectionPanel#VIDEO_MODE}
	 ...
	 * @param supportedMediaFiles list of media files supported by this recognizer
	 * @param vm the viewer manager
	 * @param enableFileSelection if {@code true} the option to select a file
	 *   is enabled, otherwise the file selection option is disabled.	
	 */
	 ...
	 * By default, the tiers radio button is selected.
	 * 
	 * @param option should be one of {@code AbstractSelectionPanel#SELECTIONS}
	 *  and {@code AbstractSelectionPanel#FILE_NAME}
	 */
	 ...
	 * 
	 * @param color the background color to be updated
	 */
	...
	/**
	 * Sets the type of file dialog which is created when the file browse
	 * button is clicked.
	 * 
	 * @param dialogType FileChooser.OPEN_DIALOG or
	...
	/**
	 * Sets the array of extensions which will be used as file filters for the
	 * file chooser dialog
	 * 
	 * @param fileExtensions a list of extension arrays
	 */
	 ...
	 * 
	 * @return the mode, one of {@code AbstractSelectionPanel#AUDIO_MODE} and 
	 * 				{@code AbstractSelectionPanel#VIDEO_MODE}
	 */
	@Override
	public int getMode(){
		return avMode;
	 ...
	 * 
	 * @param mediaFilePaths a list of media file paths
	 */
	...
	/**
	 * Updates the list of tier names after a tier was added, deleted or changed.
	 * 
	 ...
	 * 
	 * @return a map with one or two key-value pairs, depending on the mode
	 */
	@Override
	 ...
	 * 
	 * @return {@code ArrayList<RSelection>} or {@code String} or {@code null}
	 */
	@Override
	 ...
	 * @return a map which can be stored by ELAN preferences
	 */
	@Override
	 ...
	 * Set the value for current parameters. 
	 * 
	 * @param map a map which contains the parameter name(s) and its/their value(s)
	 */
	@Override
	...
	/**
	 * Panel to select time selections.
	 * 
	 * @author Han Sloetjes
	 */
	private class SelectionPanel extends JPanel implements ActionListener {
		...
		/**
		 * Updates the UI for stereo or mono mode, depending on the sound file
		 * in the wave viewer. 
		 * By default stereo mode is off, selections are made for a single 
		 * (or all) channels. 
		 * 
		 */
		 ...
		 * 
		 * @return {@code true} if currently segments for the left and right
...
//					popup.show(selectionList, p.x, p.y);
		 ...
		 * 
		 * @return {@code true} if there is at least one selection, 
		 ...
		 * 
		 * @param enabled if {@code true} the UI elements are enabled, otherwise
		 * they are disabled		 
		 */
		@Override
		...
		class ShapeAction extends AbstractAction {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (player == null) {
					List<ElanMediaPlayer> connected = vm.getConnectedMediaPlayers();
