	...
	private void addChildElement(CMDIMetadataElement mdElement, DefaultMutableTreeNode parentNode) {
		...
		try {
			...
			if (numTypeExceptions <= 1) {
				LOG.warning("Unable to retrieve data category information: " + t.getMessage());
			}
			numTypeExceptions++;
		}
		...
		public void run() {
			...
			try {
				...
				try {
		            ...
		            LOG.warning("Metadata exception: " + me.getMessage());
				} catch (IOException ioe) {
					LOG.warning("Metadata io exception: " + ioe.getMessage());			
				} catch (Throwable th) {
					LOG.warning("Metadata loading exception: " + th.getMessage());
				}
				...
				if (cmdiDoc == null) {
					try {
						...
						LOG.warning("CMDI DOM throwable: " + t.getMessage());
					}
				}
			} finally {
				treeLock.unlock();
