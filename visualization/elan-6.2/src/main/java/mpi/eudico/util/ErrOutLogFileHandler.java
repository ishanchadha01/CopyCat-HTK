...
public class ErrOutLogFileHandler extends StreamHandler {
     ...
     * from a configuration file, or using default values when no configuration
...
//			}
    ...
    /**
     * Initializes the Handler. Tries to find or create a file that can be
    ...
    protected void readConfiguration() {
        ...
        if (formatString != null) {
            try {
                ...
                formatter = (Formatter) formClass.getDeclaredConstructor().newInstance();
            } catch (Exception e) {
        ...
        if (levelString == null) {
        	levelString = manager.getProperty(".level"); // global level
        }
        if (levelString != null) {
            try {
