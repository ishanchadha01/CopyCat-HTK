...
from .feature_extraction_hands import feature_extraction_hands
...
def extract_features(features_config: dict, ark_dir: str, verbose: bool, is_select_features: bool,
   ...
   ark_dirname = os.path.join(ark_dir, word)
   ark_filepath = os.path.join(ark_dirname, ark_filename)
   title = ark_filename.replace('.ark', "")
   
   if True: # temporary conditional to handle just mp hands extraction
      features_df = feature_extraction_hands(features_filepath, features_config['selected_features'], drop_na=True)
   #TODO: make this an arg rather than parsing name
   elif 'alphapose' in features_filename:
   ...
   if features_df is not None:
       ...
       if not os.path.exists(ark_dirname):
           os.makedirs(ark_dirname)
       if num_rows > 0:
