...
KALDI_DIR = '/home/ishan/Documents/research/ccg/kaldi/build/'  # MODIFY AS NEEDED

def run_kaldi_command(htk_dir: str, ark_file: str):
    htk_script_file = os.path.join(KALDI_DIR, 'src/featbin/copy-feats-to-htk')
    word = ark_file.split('/')[-2]
    htk_dir_nested = os.path.join(htk_dir, word)
    kaldi_command = (f'{htk_script_file} '
                     f'--output-dir={htk_dir_nested} '
                     f'--output-ext=htk '
